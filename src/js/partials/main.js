window.addEventListener('load', () => {
  ...
  let eventType = 'ontouchstart' in window ? 'ontouchstart' : 'onclick';
  ...
  window.addEventListener('resize', () => {
    ...
    eventType = 'ontouchstart' in window ? 'ontouchstart' : 'onclick';
  ...
  const resultSectionAside = resultSection.querySelector('.result-section__aside');
  const resultImages = document.querySelectorAll('.result-section__aside img');
    ...
    if (sectorsKeys.length) {
      ...
      if (resultSection.classList.contains('invisible')) {
        ...
        resultSectionContent.ontransitionend = () => {
          ...
          resultSectionAside.classList.remove('invisible');
          fullpage_api.reBuild();
        }
        resultSectionContent.classList.add('invisible');
        resultSectionAside.classList.add('invisible');
      ...
      function fillWithContent() {
        resultImages.forEach(it => {
          it.classList.contains(key) ? it.classList.add('active') : it.classList.remove('active');
        });
        ...
        resultSection.querySelector('.__js_retry')[eventType] = resetResult;
        const moreBtn = resultSection.querySelector('.__js_more');

        if (sectorsKeys.length) {
          moreBtn[eventType] = showResult;
